...
public class ScannerElement : SingletonMonoBehavior<ScannerElement>
{
    public Game game;
    ...
    public void OnGameUpdate(Game game)
    {
        ...
        {
            transform.DOMoveY(
                chart.GetScannerPositionY(game.Time, game.Config.UseScannerSmoothing),
                Time.deltaTime
            ).SetEase(Ease.Linear);
        }
        
        // Direction
        Direction = chart.CurrentPageId < chart.Model.page_list.Count
            ? chart.Model.page_list[chart.CurrentPageId].scan_line_direction
            : -chart.Model.page_list[chart.CurrentPageId - 1].scan_line_direction;
    }
}